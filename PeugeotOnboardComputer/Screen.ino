...
void screenSensors() {
  if (!isDrawNames) {
    tft.clearScreen();
    tft.setColor(0, colRed, colGreen, colBlue);
    drawNames("Voltmeter", "Tank level", "Tank level now", "Tank. raw data");
    isDrawNames = true;
    pos_y = 0;
    tft.setFont(ucg_font_8x13B_mr);
...
void screenOther() {
  if (!isDrawNames) {
    tft.clearScreen();
    tft.setColor(0, colRed, colGreen, colBlue);
    drawNames("Consumption", "RPM", "Inj. time", "Temp inside");
    isDrawNames = true;
    pos_y = 0;
    tft.setFont(ucg_font_8x13B_mr);
